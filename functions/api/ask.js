...
function normalizeExtractedCase(extracted) {
  ...
  // Ensure each medication.name is a string
...
function safeParseJSON(raw) {
  ...
  for (let i = 0; i < cleaned.length; i++) {
    if (cleaned[i] === "{") { if (start === -1) start = i; depth++; }
      ...
      try { return JSON.parse(cleaned.substring(start, i + 1)); } catch (_) { break; }
    }}
  }
  console.error("safeParseJSON: could not parse, raw length:", raw?.length);
  return {};
}
