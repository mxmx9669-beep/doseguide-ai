...
// Uses env: OPENAI_API_KEY, VECTOR_STORE_ID, MODEL (optional)
...
export async function onRequest(context) {
  ...
  async function openaiFetch(endpoint, bodyObj) {
    must(OPENAI_API_KEY, "Missing OPENAI_API_KEY in Pages env", 500);
  ...
  async function vectorSearch(query, maxResults = 8) {
    must(VECTOR_STORE_ID, "Missing VECTOR_STORE_ID in Pages env", 500);
  ...
  async function generateAnswer({
    ...
    const userPrompt = [
      ...
      ...evidence.map((e) => {
        const pageStr =
          e.page !== null && e.page !== undefined ? ` | page:${e.page}` : "";
    ...
    try {
      ...
      parsed = {
        verdict: "NOT_FOUND",
        answer: "Parser error: model output was not valid JSON.",
        citations: [],
      };
    }
  ...
  // =========================
  // Routes inside single file
  // =========================
  try {
    const url = new URL(request.url);
    const path = url.pathname; // /api/ask

    // Health via query: /api/ask?health=1 (اختياري)
    if (request.method === "GET" && (url.searchParams.get("health") === "1")) {
      return text("DoseGuide Pages Function OK");
    }

    // Optional: /api/ask?catalog=1 -> list vector store files
    if (request.method === "GET" && (url.searchParams.get("catalog") === "1")) {
      must(VECTOR_STORE_ID, "Missing VECTOR_STORE_ID", 500);
      const list = await openaiGet(
        `/vector_stores/${encodeURIComponent(VECTOR_STORE_ID)}/files?limit=100`
      );
    ...
    // Main ask endpoint: POST /api/ask
    if (request.method !== "POST") {
      return json(
        { ok: false, error: "Not found. Use POST /api/ask" },
        404
      );
    }
    ...
    const outputMode = normalizeOutputMode(
      body.output_mode || body.received_mode || body.mode
    );
    ...
    if (!s.ok) {
      return json(
        { ok: false, error: "VECTOR_SEARCH_FAILED", details: s.error },
        502
      );
    }
